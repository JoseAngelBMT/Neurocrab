    ...
    for node in tape.nodes.iter().rev() {
        if let Some(grad_out) = grad_store.remove(&node.output) {
            let grads: Vec<Tensor<T>> = match node.op {
                ...
                OpKind::Sigmoid => {
                    let a: Tensor<T> = get_saved_tensor(&node.saved, 0);
                    let result: Tensor<T> = grad_out
                        .mul(&a)
                        .unwrap()
                        .mul(&Tensor::ones(a.shape().to_vec()).sub(&a).unwrap())
                        .unwrap();
                    vec![result.clone()]
                }
            };
...
mod tests {
    ...
    fn matmul_autograd() {
        ...
        assert_eq!(
            grad_store.get(&a_v.id).unwrap().data(),
            &vec![11, 15, 11, 15]
        );
    ...
    fn relu_autograd() {
        ...
        let x_v = tape.variable(x);
        let y = x_v.relu(&mut tape);

        let initial_grad = Tensor::ones(vec![2]);
        let grad_store = backward(&tape, y.id, initial_grad);
        assert_eq!(grad_store.get(&x_v.id).unwrap().data(), &vec![0.0, 1.0]);
    }
    ...
    fn sigmoid_autograd() {
        ...
        let x_v = tape.variable(x);
        let y = x_v.sigmoid(&mut tape);

        let initial_grad = Tensor::ones(vec![1]);
        let grad_store = backward(&tape, y.id, initial_grad);
        assert_eq!(grad_store.get(&x_v.id).unwrap().data(), &vec![0.25]);
    }

    #[test]
    fn mean_autograd() {
        let mut tape = Tape::new();
        let x = Tensor::from_vec(vec![2.0, 4.0], vec![2]).unwrap();
        let x_v = tape.variable(x);
        let y = x_v.mean(&mut tape);
